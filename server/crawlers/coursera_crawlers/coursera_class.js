...
async function getCoursera() {

    const getAutoStatus = await query("SELECT time, status, run FROM pangtingder.auto_work WHERE work = 'coursera'")
    ...
    if(getStatus){
        ...
        if(!+getRun){
            ...
            let setTime = "UPDATE `pangtingder`.`auto_work` SET `time` = ? WHERE (`work` = 'coursera');"
            await query(setTime, nowTime)

            await query("UPDATE pangtingder.auto_work SET run = 1  WHERE (work = 'coursera')")
            ...
            const getAllClass = JSON.parse(JSON.stringify(await query("SELECT number FROM pangtingder.class WHERE number like 'course%'")))

            // 獲得目前課程更新到的位置，第一次爬蟲要拿到所有課程。
            ...
            const page = async (num, witchClass) =>{
                ...
                if(maxTime){
                    ...
                    await query("UPDATE pangtingder.auto_work SET run = 0, msg = '課程更新完成' WHERE (work = 'coursera')")
                    console.log('end')
                    return
                }
